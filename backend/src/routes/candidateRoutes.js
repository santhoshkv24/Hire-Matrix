const express = require("express");
const path = require("path");
const fs = require("fs");
const multer = require("multer");
const {
  createCandidate,
  listCandidates,
  getCandidateById,
  updateCandidate,
  deleteCandidate,
  uploadResume,
  downloadResume,
} = require("../controllers/candidateController");
const { authenticate, authorize } = require("../middlewares/auth");
const { ROLE_KEYS } = require("../config/constants");
const HttpError = require("../utils/httpError");

const resumeDir = path.resolve(process.cwd(), "uploads", "resumes");
fs.mkdirSync(resumeDir, { recursive: true });

const storage = multer.diskStorage({
  destination: (_req, _file, cb) => cb(null, resumeDir),
  filename: (_req, file, cb) => {
    const sanitizedName = file.originalname.replace(/\s+/g, "-").toLowerCase();
    cb(null, `${Date.now()}-${sanitizedName}`);
  },
});

const upload = multer({
  storage,
  limits: {
    fileSize: 10 * 1024 * 1024,
  },
  fileFilter: (_req, file, cb) => {
    const allowedMime = [
      "application/pdf",
      "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      "application/msword",
      "text/plain",
    ];

    if (!allowedMime.includes(file.mimetype)) {
      cb(new HttpError(400, "Unsupported resume file type"));
      return;
    }

    cb(null, true);
  },
});

const router = express.Router();

router.use(authenticate, authorize(ROLE_KEYS.ADMIN, ROLE_KEYS.RECRUITER, ROLE_KEYS.HIRING_MANAGER));

router.get("/", listCandidates);
router.get("/:candidateId", getCandidateById);
router.get(
  "/:candidateId/resumes/:resumeId/download",
  authorize(ROLE_KEYS.ADMIN, ROLE_KEYS.RECRUITER, ROLE_KEYS.HIRING_MANAGER),
  downloadResume
);
router.post("/", authorize(ROLE_KEYS.ADMIN, ROLE_KEYS.RECRUITER), createCandidate);
router.patch("/:candidateId", authorize(ROLE_KEYS.ADMIN, ROLE_KEYS.RECRUITER), updateCandidate);
router.delete("/:candidateId", authorize(ROLE_KEYS.ADMIN, ROLE_KEYS.RECRUITER), deleteCandidate);
router.post(
  "/:candidateId/resume",
  authorize(ROLE_KEYS.ADMIN, ROLE_KEYS.RECRUITER),
  upload.single("resume"),
  uploadResume
);

module.exports = router;